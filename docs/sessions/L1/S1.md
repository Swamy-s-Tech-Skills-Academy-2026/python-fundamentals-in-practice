...
PS C:\Users\YourName> py -0p
 -V:3.13 *        C:\Users\YourName\AppData\Local\Programs\Python\Python313\python.exe
...
![Python Source Code to Bytecode Compilation](../../images/S1/Py_Source_ByteCode.PNG)
...
- 🔄 **Makes Python portable:** your bytecode can run on any Windows machine
...
- **Portable** - Same code works across different Windows systems
...
- [ ] This process makes Python portable
...
### 🔍 Optional: Check Syntax with `py_compile`

As you start writing more Python files, it's useful to check that your code has **no syntax errors** (typos that stop Python from even running the file).

Python ships with a built-in module called `py_compile` that can **compile your script to bytecode** and report syntax problems.

From the **repo root** (`python-fundamentals` folder), you can run:

```powershell
python -m py_compile src/L1/S1/02_interactive_hello.py
```

If there are **no syntax errors**, this command finishes silently and creates a compiled `.pyc` file under `src/L1/S1/__pycache__/`.

If there **is** a syntax error, you'll see an error message showing:

- The file name (`02_interactive_hello.py`)
- The line number
- A short description of what went wrong

This is a great habit to build: **compile your file to catch syntax issues early**, especially before sharing code or committing changes.
